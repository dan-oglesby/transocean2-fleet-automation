...
- A single `Auto-pilot all` toggle enables/disables TO2 Captain mode for every visible ship at once and shows its current on/off state; toggling it refreshes the ship list immediately.
...
- Auto upgrades can route idle ships to real upgrade docks and start the game's native upgrade flow only after treasury is above the fleet cushion, which is the higher of 20,000,000 per visible ship or a 200,000,000 total floor.
