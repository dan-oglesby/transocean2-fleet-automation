...
- Before any automated departure (contract dispatch, service-dock routing, or idle repositioning), the mod estimates fuel remaining on arrival with the native `ShipFactory.GetRemainingFuelOnArrival(...)` path. If needed, it buys enough fuel plus a small 3% tank reserve through native `RefuelPlayerShip(...)` pricing/credit deduction before calling movement.
- If a ship remains in `MINIGAME_CASTOUT` with a current harbor and destination after the dispatch settle window, automation treats it as a stale departure, verifies fuel again, and retries the same destination no more than once every 20 real seconds.
- Before any enabled ship accepts more work, the mod now reconciles native onboard contracts with `GetAllJobsFromPlayerShip`: already-assigned cargo is never treated as available work, deliverable onboard cargo is forced through the native unload path, and destination-mismatched cargo makes the ship sail to that destination. If the ship is docked and has spare capacity, it may top up with additional legal same-destination cargo from the current harbor before sailing.
...
- Auto repairs can send low-condition ships to the nearest safe reachable repair dock, requiring arrival condition to stay at least 10% above the native sink-risk threshold when possible. If no repair dock meets that cushion, the router may use an emergency no-sink-risk dock below the cushion rather than stranding the ship indefinitely.
...
- Repair and upgrade routing now prefers company-owned subsidiary harbors when the native `GetHarborSubsidiaryOwner` data says the destination harbor belongs to the player. Repair selection treats owned repair docks as modestly closer; upgrade selection adds a bounded score bonus so owned ports win close calls without overriding much better upgrades.
...
- Blocking tugboat-strike and customs-search popups are watched during active games. If one stays open for 25 real seconds, the mod chooses the safe native continuation path (`Wait` for tugboat strikes, close/resume for customs searches) so unattended automation can keep moving.
- Large-fleet newsticker spam is gently condensed for ship arrival/job feedback, scanner/contraband, repair-complete, and upgrade-complete notices: the three newest notices of each type stay visible and clickable, while older duplicates are removed after a 90-second grace period.
...
Auto-upgrade automation is experimental. It uses the native upgrade bridge, keeps a fleet cushion untouched (the lesser of 10,000,000 credits per visible ship or a 50,000,000 total cap), and only starts one upgrade per shipyard visit. Until that cushion is reached, ships may still repair and haul contracts, but they will not start or route toward upgrades. If no affordable upgrade is available above the cushion, or if a ship is already more than one upgrade ahead of the enabled fleet's low-water mark, the ship continues normal contract automation.

The popup watchdog is also experimental. It only runs after an active player fleet is loaded, and it deliberately waits 25 real seconds before dismissing tugboat-strike or customs-search UI. Newsticker consolidation uses the game's native item removal method, not a true expandable archive; it keeps the latest notices available for inspection and removes older duplicates after a delay.
